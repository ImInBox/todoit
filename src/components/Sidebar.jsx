...
function Sidebar(){
    ...
    return(
            ...
            <div className="flex h-full w-[300px] flex-col">
                ...
                <ul>
                    {sidebarItems.map((item, index) => (
                        <li key={index} onClick={() => navigate(item.link)} className="rounded duration-100 hover:bg-slate-700 hover:text-white m-2 p-2 flex items-center cursor-pointer">
        ...
        <button  onClick={() => setIsOpen(!isOpen)} className={`${isOpen ? "translate-x-[300px]" : ""} group px-2 py-3 rounded duration-300 hover:text-white hover:bg-slate-700 m-4 top-0 left-0 bg-white fixed`}>
